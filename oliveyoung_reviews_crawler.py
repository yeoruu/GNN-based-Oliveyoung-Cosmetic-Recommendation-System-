...
OUT_TABLE1 = "table1_essence_reviews.csv"
...
def set_sort_latest(page, verbose=False):
    """리뷰 정렬을 '최신순'으로 설정"""
    if verbose:
        print("    🔍 최신순 정렬 버튼 찾는 중...")
    
    # 정렬 옵션들이 수평으로 나열된 형태에서 '최신순' 클릭
    ...
    for sel in candidates:
        ...
        if loc.count() > 0:
            try:
                ...
                if verbose:
                    print(f"    ✅ 최신순 정렬 성공: {sel}")
                return
            except Exception as e:
                if verbose:
                    print(f"    ⚠️  최신순 정렬 실패 ({sel}): {str(e)[:50]}")
                continue
    
    if verbose:
        print("    ⚠️  최신순 정렬 버튼을 찾을 수 없음 (계속 진행)")
...
def collect_latest_reviews_for_product(page, product_url, limit=None, verbose=False):
    """제품 페이지에서 최신순 리뷰 수집"""
    ...
    set_sort_latest(page, verbose=verbose)
    ...
    if verbose:
        print(f"    📋 리뷰 수집 시작... (목표: 전체 리뷰 {limit if limit else 100}개)")
    collected = []
    seen = set()
    guard = 0
    iteration = 0
    ...
    while len(collected) < target_count and guard < 30 and iteration < max_iterations:
        ...
        if n == 0 and verbose:
            ...
            # 디버깅: 페이지의 주요 요소 확인
            try:
                review_section = page.locator('text=리뷰').first
                if review_section.count() > 0:
                    print("      ℹ️  '리뷰' 텍스트는 발견됨")
                else:
                    print("      ⚠️  '리뷰' 텍스트를 찾을 수 없음")
                
                # 신고하기 텍스트 확인
                report_links = page.locator('text=신고하기')
                report_count = report_links.count()
                print(f"      ℹ️  '신고하기' 링크: {report_count}개")
                
                # 리뷰 관련 다른 요소들 확인
                li_count = page.locator("li").count()
                div_count = page.locator("div").count()
                div_inner_count = page.locator("div.inner").count()
                oy_review_content_count = page.locator("oy-review-review-content").count()
                print(f"      ℹ️  페이지 요소: li {li_count}개, div {div_count}개, div.inner {div_inner_count}개")
                print(f"      ℹ️  oy-review-review-content: {oy_review_content_count}개")
                
                # 리뷰 탭이 활성화되어 있는지 확인
                active_tab = page.locator('a:has-text("리뷰"), button:has-text("리뷰")').filter(has=page.locator('.on, .active, [class*="selected"]'))
                if active_tab.count() > 0:
                    print("      ✅ 리뷰 탭이 활성화되어 있음")
                else:
                    print("      ⚠️  리뷰 탭 활성화 상태 불명확")
                
                # 페이지 스크린샷 저장 (디버깅용)
                if iteration == 1:
                    try:
                        page.screenshot(path="debug_review_page.png", full_page=True)
                        print("      💾 디버깅용 스크린샷 저장: debug_review_page.png")
                        
                        # HTML 일부 저장 (디버깅용)
                        html_snippet = page.content()[:5000]  # 처음 5000자만
                        with open("debug_review_page.html", "w", encoding="utf-8") as f:
                            f.write(html_snippet)
                        print("      💾 HTML 스니펫 저장: debug_review_page.html")
                    except Exception as e:
                        if verbose:
                            print(f"      ⚠️  디버깅 파일 저장 실패: {str(e)[:50]}")
            except Exception as e:
                if verbose:
                    print(f"      ⚠️  디버깅 정보 수집 실패: {str(e)[:50]}")
        ...
        for i in range(n):
            ...
            if not review_text:
                skipped_no_text += 1
                if verbose and skipped_no_text <= 3:  # 처음 몇 개만 로그
                    # 리뷰 텍스트가 없는 이유 디버깅
                    try:
                        card_text = card.inner_text()[:200]
                        print(f"      ⚠️  카드 {i+1}: 리뷰 텍스트 없음 (user_id: {user_id}, 텍스트 샘플: {card_text[:50]}...)")
                    except:
                        pass
                continue
        ...
        if verbose:
            print(f"      └─ 새로 추가: {parsed_count}개, 텍스트 없음: {skipped_no_text}개, 중복: {skipped_duplicate}개")
...
def test_single_product(product_url=None, headless=False, verbose=True, limit=120):
    ...
    print(f"목표 리뷰 수: {limit}개")
    print(f"Headless 모드: {headless}")
    print("="*60)
    print("\n💡 디버깅 팁:")
    print("   - 리뷰 수집이 안 되면 headless=False로 실행하여 브라우저를 확인하세요")
    print("   - 개발자 도구(F12)에서 Elements 탭으로 리뷰 카드의 HTML 구조를 확인하세요")
    print("   - '신고하기' 텍스트가 있는 요소를 찾아 그 부모 요소의 클래스/구조를 확인하세요")
    print("   - 디버깅 파일이 생성됩니다: debug_review_page.png, debug_review_page.html\n")
    ...
    with sync_playwright() as p:
        ...
        try:
            product_name, product_rating, reviews = collect_latest_reviews_for_product(
            ...
            print(f"수집된 리뷰 수: {len(reviews)}개")
            print("="*60)
            ...
            if reviews:
                ...
                print("="*60)
                print("\n🔍 문제 해결 방법:")
                print("1. 브라우저가 열려있다면 페이지를 직접 확인하세요")
                print("2. 개발자 도구(F12)를 열어 Elements 탭에서 확인:")
                print("   - '신고하기' 텍스트를 검색(Ctrl+F)")
                print("   - 그 요소의 부모 요소 구조 확인")
                print("   - 리뷰 카드의 클래스명이나 구조 파악")
                print("3. 생성된 디버깅 파일 확인:")
                print("   - debug_review_page.png (스크린샷)")
                print("   - debug_review_page.html (HTML 스니펫)")
                print("4. 리뷰 카드 선택자를 수정해야 할 수 있습니다")
                print("="*60)
                return None
...
def scrape_all_products_reviews(headless=True, verbose=True, start_idx=0, end_idx=None):
    """
    모든 제품의 리뷰 크롤링 (목표: 각 제품당 전체 리뷰 100개)
    ...
    if verbose:
        ...
        print(f"목표: 각 제품당 전체 리뷰 {REVIEWS_PER_PRODUCT}개")
...
if __name__ == "__main__":
    ...
    if test_mode:
        print("\n🧪 테스트 모드 활성화")
        print("💡 사용법: python oliveyoung_reviews_crawler.py --test [--headless]")
        print("   또는 URL 지정: 코드에서 product_url 인자 직접 지정\n")
        
        # URL 찾기 (명령줄에서 --url=xxx 형태로 받을 수도 있지만, 여기서는 None으로 하면 CSV 첫 번째 사용)
        test_url = None
        for arg in args:
        ...
        if df_reviews is not None and len(df_reviews) > 0:
            # 테스트 결과를 임시 파일로 저장
            test_output = "test_reviews_output.csv"
        ...
        print("\n💡 테스트 모드: python oliveyoung_reviews_crawler.py --test")
        print("\n💡 범위 지정 사용법:")
        print("   python oliveyoung_reviews_crawler.py --start=0 --end=50")
        print("   python oliveyoung_reviews_crawler.py --start=50 --end=100")
        ...
        if df_reviews is not None and len(df_reviews) > 0:
            ...
            print("📋 결과 미리보기")
            print("="*60)
            print(df_reviews[["product_id", "product_name", "product_rating", "user_id", "user_rating", "review_text"]].head(10))
