"""
단일 노드(제품) GNN 기반 화장품 추천시스템 (BPR + Negative Sampling 버전)

- 노드(제품) 피처: 카테고리 one-hot + 브랜드 one-hot + 성분 TF-IDF -> SVD + 피부타입별 평균평점
- 엣지: 성분 임베딩(SVD) 코사인 유사도 기반 + 같은 카테고리 + (옵션) 협업 규칙
- 학습: BPR loss (pos: rating>=4, neg: user가 train에서 안 본 아이템 랜덤)
- 평가: 진짜 추천 평가 (전체 611 후보에서 train-seen 제외 후 top-k)
"""
...
class SingleNodeGNNRecommender:
    ...
    def _preprocess_data(self):
        ...
        self.ingredient_svd = normalize(ing_svd)                    # cosine 안정화
    ...
    def _build_graph(self):
        ...
        # ✅ 노드 피처: category + brand + ingredient_svd + skin_ratings
    ...
    def _compute_edges(self, top_k=15):
        # ingredient_svd는 이미 normalize -> dot = cosine
    ...
    def evaluate(self, k=5, max_users=2000, seed=42):
        ...
        for u in users:
            ...
            user_feature = torch.FloatTensor([user_profiles[int(u)]]).to(self.device)  # [1,8]

            with torch.no_grad():
                # 전체 아이템 스코어 (1, N) 만들기 위해 user_feature를 N개로 반복
                u_rep = user_feature.repeat(len(self.products_df), 1)
                scores = self.model.score(self.data.x, self.data.edge_index, u_rep, all_items).detach().cpu().numpy()
    ...
    def recommend(self, skin_type, category=None, favorite_product_id=None, top_n=5):
        # 카테고리 5개 고정(원하면 여기서만 선택하게 UI 만들어도 됨)
        ...
        # 유저 피처(8): skin one-hot(혹은 분포) + rating_feat(상수)
        ...
        if favorite_product_id and favorite_product_id in self.products_df["product_id"].values:
            ...
            sim = (fav_vec @ self.ingredient_svd.T).ravel()  # cosine (normalize됨)
    ...
    def print_recommendations(self, recs):
        ...
        for i, r in enumerate(recs, 1):
            ...
            print(f"\n   💬 같은 피부타입 사용자 리뷰:")
            for j, review in enumerate(r["reviews"], 1):
                short = review[:100] + "..." if len(review) > 100 else review
                print(f"      {j}) {short}")
            print("-" * 80)
    ...
    def save(self, save_dir="singlenode"):
        ...
        print("\n✅ 저장 완료")
        print(" - torch:", os.path.join(save_dir, "single_node_gnn_recommender_bpr.pt"))
        print(" - joblib:", os.path.join(save_dir, "tfidf_vectorizer.joblib"), os.path.join(save_dir, "svd.joblib"))
        print(" - npy:", os.path.join(save_dir, "ingredient_svd.npy"))
...
def main():
    ...
    # ✅ 추천 테스트 (카테고리 5개 중 하나로!)
    print("\n\n" + "=" * 80)
    print("💡 추천 시스템 테스트")
    print("=" * 80)

    recs = recommender.recommend(skin_type="민감성", category="에센스/세럼/앰플", favorite_product_id=None)
